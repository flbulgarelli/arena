...
trait DialogTrait[T] extends SimpleWindow[T] with TaskWindow {
	...
	override def onAccept(action:Action) = this.acceptActions.append(action)

	override def onCancel(action:Action) = this.cancelActions.append(action);
	...
	def changeStateToCancelled() = this.state = CANCELLED
