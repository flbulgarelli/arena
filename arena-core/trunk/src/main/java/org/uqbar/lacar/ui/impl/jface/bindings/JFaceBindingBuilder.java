...
public class JFaceBindingBuilder implements BindingBuilder {
	private final JFaceWidgetBuilder<?> widget;
	...
	public void observeProperty(ObservableObject model, String propertyName) {
		this.model = observeValue(model, propertyName);
	}

	// Se pierden los tipos de los adapters, Java no tiene una forma de evitar esto.
	...
	@Override
	public void build() {
		this.widget.getDataBindingContext().bindValue(this.view, this.model, this.viewToModel, this.modelToView);
	}
	...
	public PropertyDescriptor getPropertyDescriptor(Class beanClass,
			String propertyName) {
		BeanInfo beanInfo;
		try {
